...
/**
 * Default to the currently selected model. If it is unavailable, fallback to
 * Codex mini, then haiku when those are configured and authenticated.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api>> {
	const currentAuth = await modelRegistry.getApiKeyAndHeaders(currentModel);
	if (currentAuth.ok) {
		return currentModel;
	}
